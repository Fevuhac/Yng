...
function genGoldHistory(cb) {
    ...
    RedisUtil.multi(data, function (err, res) {
        ...
        for (var i = 0; i < QUERY_MAP.length; i++) {
            
            var hashkey = QUERY_MAP[i] + ":history";
            var date = DateUtil.getCurrentByFormat('yyyy-MM-dd');
...
function queryGold(start_date, end_date, cb) {
    ...
    for (var i = 0; i < date_list.length; i++) {
        ...
        var timestamp = new Date(date_list[i]).getTime();
    ...
    RedisUtil.multi(data, function (err, res) {
        ...
        for (var i = 0; i < res.length; i = i + QUERY_MAP.length) {
