...
class RedisSync {
    ...
    parseValue(key, value) {
        ...
        if (!typeInfo) {
            console.log('parseValue,非法字段，请检查字段名是否正确', __filename, key)
            return null;
        }
    ...
    static buildValue(key, value) {
        ...
        let typeInfo = account_def.getField(key);
        if (!typeInfo) {
            console.log('buildValue,非法字段，请检查字段名是否正确', __filename, key)
            return '';
        }
        switch (typeInfo.type) {
            case 'float':
    ...
    //计算盈亏系数
    _calcGainLoss(){
        return (this.cash * common_const_cfg.CHANGE_CASH_4 + this.gold + this.cost) - (this.recharge * common_const_cfg.CHANGE_CASH_5 + player_users_cfg[0].gold);
    }
    ...
    commit(cb) {
        ...
        RedisUtil.multi(cmds, function (err, result) {
            ...
            RedisUtil.sadd('fishjoy:updated_uids', this.id);
            if(gainLossChange){
                let v = this._calcGainLoss();
                RedisUtil.zadd('rank:gain', v, this.id);
                RedisUtil.zadd('rank:loss', v, this.id);
            }
    ...
    get cost() {
        return this._value(account_def.AccountDef.cost.name);
    }

    set cost(value) {
        this._modify(account_def.AccountDef.cost.name, value);
    }
    get bonus_pool() {
        return this._value(account_def.AccountDef.bonus_pool.name);
    }

    set bonus_pool(value) {
        this._modify(account_def.AccountDef.bonus_pool.name, value);
    }
    get pump_pool() {
        return this._value(account_def.AccountDef.pump_pool.name);
    }
...
function setAccountById(id, data, cb) {
    ...
    if (fields.length > 1) {
        ...
        RedisUtil.multi(cmds, function (err, result) {
            ...
            // console.log('setAccountById multi docs result:', result);
    ...
    _exist(id, function (err) {
        ...
        if (allField || !!fields && fields.length > 1) {
            ...
            RedisUtil.hget(`pair:uid:${fields[0]}`, id, function (err, doc) {
                ...
                if (DEBUG) console.log('single:', fields[0], '=', doc);
