...
function getChange(account, rewardInfo) {
    ...
    if (DEBUG) {
        ...
        console.log(FUNC + "skin_inc:", skin_inc);
        console.log(FUNC + "skin_debris_inc:", skin_debris_inc);
    }
    ...
    if (skin_debris_inc && _.keys(skin_debris_inc).length > 0) {
        ...
        for (var idx in skin_debris_inc) {
...
function _cacheLink(data, api_name, api_type) {
    ...
    RedisUtil.hset(PAIR.UID_LAST_ONLINE_TIME, uid, timestamp);
    RedisUtil.hset(CACHAE.UID_TOBE_STORED, uid, 1);
}
...
function getChartRewardByTypeAndRank(type, rank) {
    for (var i = 0; i < rank_ranklist_cfg.length; i++) {
        var info = rank_ranklist_cfg[i];
