...
var ERROR = 1;
var TAG = "【RedisUtil】";
const KEY_ACC_COUNTER = 'account_counter';
...
exports.generateNewId = function (cb) {
    client.incr(PAIR.KEY_ACC_COUNTER, function (err, count) {
        cb && cb(count);
    });
};
...
function hincrby(hashkey, field, num, cb) {
    ...
    function logInfo() {
        console.error(FUNC + "hashkey:", hashkey);
        console.error(FUNC + "field:", field);
        console.error(FUNC + "num:", num);
    }
}
...
function hset(hashkey, field, value, cb) {
    ...
    if (typeof(hashkey) != 'string') {
        console.log(FUNC + "typeof(hashkey):", typeof(hashkey));
        logInfo();
    }
    if (typeof(field) != 'number' && typeof(field) != 'string') {
        console.log(FUNC + "typeof(field):", typeof(field));
        logInfo();
        throw new Error("field == ", field);
    }
    if (typeof(value) != 'number' && typeof(value) != 'string') {
        console.log(FUNC + "typeof(value):", typeof(value));
        logInfo();
    }

    if (ArrayUtil.isArray(value)) {
        console.error(FUNC + "输入是数组");
        logInfo();
    }

    if (!ArrayUtil.isArray(value) && !StringUtil.isString(value) && typeof(value) != 'number') {
        console.error(FUNC + "输入是对象");
        logInfo();
    }
    ...
    function logInfo() {
        console.error(FUNC + "hashkey:", hashkey);
        console.error(FUNC + "field:", field);
        console.error(FUNC + "value:", value);
    }
}
...
function zrevrangewithscores(key, start, stop, cb) {
    ...
    if (isReadyAndConnected(cb)) {
        ...
        for (var i = start; i < count;) {
            var temp_stop = i + GAP - 1;
            var one_stop = temp_stop > count ? count : temp_stop;
    ...
    for (var i = 0; i < data.length; i++) {
        if ('hset' == data[i][0]) {
            ...
            if ('number' != typeof(val) && 'string' != typeof(val)) {
                // console.log(FUNC + "批量参数:", data[i]);
                // console.log(FUNC + "typeof(val):", typeof(val));
                // console.log(FUNC + "val.constructor:", val.constructor);
