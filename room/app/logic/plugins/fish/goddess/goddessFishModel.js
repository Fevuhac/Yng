...
class GoddessFishModel extends FishModel {
    ...
    _selectWaveFish () {
        ...
        for (var i = 0; i < fishCFG.length; i ++) {
            let fish = fishCFG[i];
            let data = {
                fishTypeKey: fish[0],
                fishInternal: fish[1],
                ...
                if (temp.count < temp.fishTotal) {
                    ...
                    if (fishTypeKey.indexOf('_json') > 0) {
                        newFishData = this._newGroupFish(fishTypeKey, function (dt, subfishes) {
                            for (var k in subfishes) {
                                let sfish = subfishes[k];
                                this._fishHurt[sfish.nameKey] = temp.fishHurt;
                            }
                        }.bind(this));
                    }else{
    ...
    removeActorData ( nameKey ) {
        ...
        this._emitEvent(GoddessFishModel.EventType.EVENT_FISH_LIVE, {liveC: liveC});
   }
   ...
   pass (passAll, godId) {
        this._emitEvent(GoddessFishModel.EventType.EVENT_PASS, {
   ...
   _ready2StartNextWave (isFirst) {
        ...
        var nextWave = this._startWaveIdx + 1;
        this._emitEvent(GoddessFishModel.EventType.EVENT_FISH_WAVE, {waveCount: nextWave, waveDt: WAVE_DT, isFirst: isFirst, rIdx: rIdx});
    }
